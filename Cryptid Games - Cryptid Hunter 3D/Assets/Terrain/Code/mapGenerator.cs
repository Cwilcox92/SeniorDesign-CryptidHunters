...
public class mapGenerator : MonoBehaviour
{
    ...
    public GameObject GrassBlock, WaterBlock, underPlateauBlock, PlateauBlock, OilBlock, ClearBlock, Campfire, Tent, Spawnpoint, AISpawnPoint, TreeRes,RockRes,BushRes;
   ...
   /* void SpawnLocalCamera(GameObject playerInstance) // this too, Kyle and I 
    ...
    public void GenerateWorld(int seed) 
    {
        ...
        PlaceResources(ColumnLength/2,RowLength/2,5,TreeRes,0);
        PlaceResources(ColumnLength/2,RowLength/2,4,BushRes,2); 
        PlaceResources(ColumnLength/2,RowLength/2,3,RockRes,1);
    ...
    int PlaceCamp(int campx, int campz)
    {
        ...
        //Tents
        pos = new Vector3(campx-2,1.69f,campz+6);
        Quaternion posRot = new Quaternion(0,180,0,1);
        WorldArray[(campx-2+101)/2,(campz+6+101)/2,1] = Instantiate (Tent, pos, posRot);

        pos = new Vector3(campx-6,1.69f,campz+4);
        posRot = new Quaternion(0,180,0,1);
        WorldArray[(campx-6+101)/2,(campz+4+101)/2,1] = Instantiate (Tent, pos, posRot);
        ...
        WorldArray[(campx+6+101)/2,(campz+4+101)/2,1] = Instantiate (Tent, pos, posRot);
        //campfire
        pos = new Vector3(campx, 1.9f,campz);
        ...
        for (int yc = 0; yc < height; yc++) 
        {
            for (int xc = 0; xc < width; xc++)
            {
                ...
                if (bluenoise[yc,xc] == max) 
                {
                    ...
                    {
                        if(Collectable == 0)    //for trees
                        {
                            pos = new Vector3((yc*2)-101,1,(xc*2)-101);
                            WorldArray[(yc*2)/2,(xc*2)/2,1] = Instantiate (Resource, pos, transform.rotation);
                            pos = new Vector3((yc*2)-101, 1.69f, (xc*2)-101); 
                            ItemWorld.SpawnItemWorld(pos, new Item { itemType = Item.ItemType.Wood, amount = 2}); 
                        }
                        else if (Collectable == 1) //for rocks
                        {
                            pos = new Vector3((yc*2)-101,2,(xc*2)-101);
                            WorldArray[(yc*2)/2,(xc*2)/2,1] = Instantiate (Resource, pos, transform.rotation);
                            pos = new Vector3((yc*2)-101, 1.69f, (xc*2)-101); 
                            ItemWorld.SpawnItemWorld(pos, new Item { itemType = Item.ItemType.Ore, amount = 2}); 
                        }
                        else if (Collectable == 2)   //for bushes
                        {
                            pos = new Vector3((yc*2)-101,2,(xc*2)-101);
                            WorldArray[(yc*2)/2,(xc*2)/2,1] = Instantiate (Resource, pos, transform.rotation);
                            //pos = new Vector3((yc*2)-101, 1.69f, (xc*2)-101); 
