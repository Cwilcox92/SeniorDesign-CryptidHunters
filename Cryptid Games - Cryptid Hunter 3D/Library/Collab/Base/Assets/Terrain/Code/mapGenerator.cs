...
public class mapGenerator : MonoBehaviour
{
    ...
    int seed = -1; 
    // 4278894 for worldarray camp generation testing (camp at -3,-47)
    ...
    int PlaceCamp(int campx, int campz)
    {
        ...
        temp = Instantiate (Spawnpoint, pos, transform.rotation);
        temp.transform.Rotate(0, 180.0f, 0.0f, Space.Self);
        ...
        temp= Instantiate(AISpawnPoint, pos, transform.rotation);
        temp.transform.Rotate(0, 180.0f, 0.0f, Space.Self);
        //Tents
        pos = new Vector3(campx-2,1,campz+6);
        Instantiate (Tent, pos, transform.rotation);
        ...
        Instantiate (Campfire, pos, transform.rotation);


        return 0;
    }

    void PlaceResources(int height, int width, int R, GameObject Resource, GameObject Collectable)
    {
        //create bluenoise
        ...
        for (int yc = 0; yc < height; yc++) 
        {
            for (int xc = 0; xc < width; xc++)
            {
                ...
                //place a "tree" only at the peaks
                if (bluenoise[yc,xc] == max) 
                {
                    ...
                    foreach (GameObject go in objs) 
                    {
                        if (go.transform.position == pos || go.transform.position == pos1) 
                        {
                            myObject = go;  
                            break;   
                        }
